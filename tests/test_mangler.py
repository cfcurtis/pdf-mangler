...
class TestMangler(unittest.TestCase):
    ...
    def test_map_charset(self):
        charset = "asdfJKLM'.;`0123"
        char_cats = mangler.categorize_chars(charset)
        self.assertEqual(
            char_cats,
            {
                "Ll": "asdf",
    ...
    def test_parse_unparse(self):
        ...
        title_char_cats = mangler.categorize_chars(title_charset)
        new_commands = []
        I_TITLE = 9
        I_FIRST_NAME = 12
        for i, (operands, operator) in enumerate(commands):
            if i == I_TITLE:
                for j in range(0, len(operands[0]), 2):
                    operands[0][j] = mangler.replace_text(str(operands[0][j]), title_char_cats)
